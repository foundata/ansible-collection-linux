...
- `copy`: one-time migration of the existing content onto the
  ...
  Limit: migrating nested trees is not supported. `copy`
  is refused when another managed mountpoint below this
  entry's path is unmounted and populated (one pass would
  duplicate the nested data onto the parent filesystem or
  hide it), and mounting is also refused while a nested
  mountpoint is actively mounted (the mount would hide it).
  Supporting this is deliberately out of scope for a
  provisioning role; the manual escape sequence is: apply
  the nested entry (its own migration mounts it), unmount
  it again, remove the migrated content below its source
  directory, apply this parent entry, then mount the nested
  entry again (for example by re-running the role).
